...
class StudentTimetableScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CampusScaffold(
      title: 'My Timetable',
      subtitle: '${user.department} - Sem ${user.semester}',
      icon: Icons.calendar_month_rounded,
      body: StreamBuilder<List<TimetableProject>>(
        stream: SupabaseService().getPublishedTimetables(
        ...
        builder: (context, snapshot) {
          ...
          if (timetables.isEmpty) {
            return const EmptyStateWidget(
                ...
                subtitle:
                    'Your admin hasn\'t published a timetable for your class yet.');
          }
              ...
              child: Column(children: [
                if (timetables.length > 1)
                  TabBar(
                      isScrollable: true,
                      tabs: timetables
                          .map((t) => Tab(text: t.className))
                          .toList()),
                Expanded(
                    child: TabBarView(
                        children: timetables
                            .map((t) => _TimetableView(project: t))
                            .toList())),
              ]));
        },
      ),
...
class _TimetableView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
        ...
        itemCount: project.workingDays,
        itemBuilder: (_, day) {
          final dayEntries = project.entries.where((e) => e.day == day).toList()
              ...
              children: [
                Padding(
                    ...
                    child: Row(children: [
                      ...
                      Text(
                          '${dayEntries.where((e) => !_isBreakSlot(e.slot)).length} classes',
                          style: TextStyle(
                ...
                ...List.generate(project.slotsPerDay, (slot) {
                  ...
                  if (_isBreakSlot(slot)) {
                    final ts = project.timeSlots[slot];
                    final breakName =
                        ts.breakName.isNotEmpty ? ts.breakName : 'Break';
                    return Container(
                  ...
                  final ci = project.subjects
                      .firstWhere((s) => s.name == entry.subjectName,
                          orElse: () => SubjectModel(
                              id: '',
                              name: '',
                              facultyName: '',
                  ...
                  final bg = isDark
                      ? AppTheme
                          .subjectAccents[ci % AppTheme.subjectAccents.length]
                          .withValues(alpha: 0.15)
                      : AppTheme
                          .subjectColors[ci % AppTheme.subjectColors.length];
                  ...
                  return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                            '👤 ${entry.facultyName} • 🏫 ${entry.roomId}',
                            style: TextStyle(
                                fontSize: 12,
                                color: AppTheme.subtitleColor(context))),
                        trailing: Text(_timeLabel(slot),
                            style: TextStyle(
                                color: accent,
                                fontSize: 10,
